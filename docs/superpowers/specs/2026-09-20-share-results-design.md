...
- それ以外の失敗はボタンの下に `画像を共有できませんでした` と1行出す（`.failed` の既存スタイル）。
  当初は `canvas.toBlob` の失敗と同じ文言にしていたが、この分岐では画像は作れている。作れなかったと伝えると原因を取り違えるので、共有に失敗したと言い分ける。
...
| `navigator.share` のその他の失敗 | `画像を共有できませんでした` を1行表示 |
| `canvas.toBlob` が null を返す | `画像を作れませんでした` を1行表示 |
